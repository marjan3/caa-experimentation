...
window.setVisData = function (jsonString) {
  ...
  window.setVisNodeColor(data.startNode, "#2777DD");
...
window.animateCaaExperiment = function (
  ...
  animateVisNodes(data[0].traversedVertices).then(() => {
    drawCaaIncrements(data[0].increments, happyNodesColor, sadNodesColor);
  });
  ...
  for (let counter = 1; counter < data.length; counter++) {
    ...
    setTimeout(() => {
      ...
      animateVisNodes(nodes).then(() => {
        drawCaaIncrements(increments, happyNodesColor, sadNodesColor);
...
window.drawCaaIncrements = function (
  ...
  edgesHistory.forEach((item, i, a) => {
    ...
    let color;
    if (item.weight > 0) {
      color = { color: happyNodesColor };
    } else {
      color = { color: sadNodesColor };
    }
