...
import com.mtanevski.master.gui.visualization.js.JSMembers;
import com.mtanevski.master.gui.visualization.js.JSFunctions;
...
import static com.mtanevski.master.gui.DefaultValues.GRAPH_VIS_TITLE;
...
public class WebWrapper {
    ...
    public void visualizeCaaAlgorithmTraversal(CaaAgentFactory.CaaAgentType caaAgentType, Boolean animate) {
        ...
        if(animate) {
            window.call(JSFunctions.animateCaaVertices.name(), gson.toJson(experimenter.getData().current().traversedVertices()));
        } else {
            window.call(JSFunctions.drawCaaVertices.name(), gson.toJson(experimenter.getData().current().traversedVertices()));
        }
    }
