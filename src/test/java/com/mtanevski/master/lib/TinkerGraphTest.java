...
public class TinkerGraphTest {
    ...
    public void testInitialGraph() {
        ...
        graph.traversal().V().has("label", P.eq("A")).next().vertices(Direction.BOTH).forEachRemaining(vertex -> {
            assertEquals("happy", vertex.property("type").value());
        });
        System.out.println(graph.traversal().V().has("label", "A").next().id());

        g.V().has("label", "A").next().edges(Direction.OUT).forEachRemaining(edge -> {
            System.out.println("EDGE: " + edge.inVertex());
        });
    }
}
